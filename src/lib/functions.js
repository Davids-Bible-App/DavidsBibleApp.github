 ...
 */
export function groupConsecutiveVerses(entries = [], wrap = false, returnAsArray = false, disableGrouping = false) {
  ...
  // 2. Sort the entries: Book -> Chapter -> Verse
  ...
  const formattedGroups = groups.map((group) => {
    ...
    // Format Header
    const refLabel =
      getVs(first) === getVs(last)
        ? `${getBk(first)} ${getCh(first)}:${getVs(first)}`
        : `${getBk(first)} ${getCh(first)}:${getVs(first)}-${getVs(last)}`;
